//! `Sample` — streamed power/waveform data, interfaces/decoders.md.
...
/// One decoded scalar sample. `data.csv` and `waveform.csv` (interfaces/decoders.md)
/// share this identical row shape rather than each inventing their own
/// columns.
///
/// **No longer a wire type as of schema v8** (decision 39): the
...
///
/// `unit`/`channel_id` (decision 27) disambiguate `value`: which
...
/// The physical quantity `Sample::value` is measured in (decision 27).
/// Append-only, same wire-compatibility rule as
/// `DevBenchMessage` (decision 10).
...
impl Unit {
    ...
    /// no quoting (interfaces/decoders.md leaves the exact formatting to this crate;
...
impl Sample {
    /// Header row for `data.csv`/`waveform.csv` (interfaces/decoders.md), written
    /// once per file.
    ...
    /// knows from the currently-open stream (decision 20). This
    /// is the crate's own CSV-rendering tool (spec.md §1, decision 2): Core's job
