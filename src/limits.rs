...
//!
//! decision 15. Placeholder-but-concrete: chosen without real
//! `embarch-dev-bench` hardware to size against, flagged for re-confirmation
//! once real nRF54 memory constraints are known. A value
//! proving too small in practice is a version-bumped (decision 12)
...
/// `DevBenchMessage::LogLine.text` (embarch-dev-bench decision 7); one
/// free-text log line dev-bench sends to Core instead of raw
/// interleaved bytes on the shared serial line.
pub const MAX_LOG_LINE_LEN: usize = 128;
/// `DevBenchMessage::HelloAck.firmware_version`
/// (embarch-dev-bench decision 18); a single free-form
/// identifier (e.g. `git describe` output) for whichever dev-bench build
/// replied to `Hello`.
pub const MAX_FIRMWARE_VERSION_LEN: usize = 32;
/// `DevBenchMessage::HelloAck.hardware_id` (decision 47,
/// embarch-core decision 35) — dev-bench's own factory-unique
...
/// `Study.streams` / `StudyResult.streams` (decision 39,
/// interfaces/taps.md). Deliberately small: a tap is a declared capture channel, not a
...
/// `StreamTap.name`, `StreamRef.name` (interfaces/taps.md) — also the name that
...
/// `StreamSource::Signal.name` (interfaces/taps.md) — the topology-declared
...
/// `StreamRecord.bytes` (interfaces/taps.md) — one arrival-stamped run of bytes.
...
/// `DevBenchMessage::StreamChunkBatch.records` (interfaces/taps.md) — how many
...
/// `GattServiceInfo` entries per `StepResult.gatt_services`
/// (decisions 31/32, interfaces/gatt-types.md); bounded against decision 57's
/// validated GATT table — 3 services declared, 7 total once an encrypted link reaches
/// the rest — with headroom for a DUT this crate hasn't seen yet.
pub const MAX_DISCOVERED_SERVICES: usize = 8;
/// `GattServiceInfo.characteristics` (interfaces/gatt-types.md); the same DUT's
...
/// (decision 53) — how many characteristics one selective
...
/// `StructLayout.header`/`repeat` (interfaces/decoders.md) — scalars in one group.
...
/// `StructField.name` (interfaces/decoders.md) — becomes a CSV column header.
pub const MAX_STRUCT_FIELD_NAME_LEN: usize = 20;
/// One rendered decoded-struct CSV row's *decoded columns*, and the header
/// naming them (interfaces/decoders.md). Bounded by
...
/// One rendered `gatt.csv` row (decision 36, interfaces/gatt-types.md) — sized to
...
// --- `.eap` protocol manifests (decisions 58-62) ------------
//
...
// (decision 59's split) and rides in `Study.protocols` all the way to the
...
// protocols in interfaces/eap.md with headroom, deliberately *not* against the generous
...
// [`MAX_STEPS_PER_STUDY`]'s 64 (embarch-dev-bench decision 27).
//
...
/// `Study.protocols` (decision 58) — `.eap` protocol blocks
...
/// (decision 59). Not every field of the real packet: only the
...
/// never reach an expression (decision 60 removed byte-span
...
/// `FrameMatch.eq` (decision 59's `select_if`) — the literal
...
/// (decision 60). Integers only: the `bytes` session variable
...
/// `StateDef.name`, and `ProtocolOutcome.final_state` (decision 62) — the
/// one string a protocol run reports back.
